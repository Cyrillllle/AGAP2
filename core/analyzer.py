...
months_num_fr = {1: 'Janvier', 2: 'Février', 3: 'Mars', 4: 'Avril', 5: 'Mai', 6: 'Juin',
                 7: 'Juillet', 8: 'Août', 9: 'Septembre', 10: 'Octobre', 11: 'Novembre', 12: 'Décembre'}
months_num_en = {1: 'January', 2: 'February', 3: 'March', 4: 'April', 5: 'May', 6: 'June',
    ...
    if duration != "":
        try:
            ...
            if "à " in duration:
                ...
                durationType = "closed"
            elif "depuis " in unidecode(lduration):
                months_num = months_num_fr
                posStart = lduration.find("depuis ")
                durationType = "opened"
            elif "since " in unidecode(lduration):
                months_num = months_num_en
                posStart = lduration.find("since ")
                durationType = "opened"
            ...
            if start_month == -1 and start_year != -1:
                start_month = 1
            if end_month == -1 and end_year != -1:
                end_month = 1
            ...
            if end_date < start_date:
                nb_months = round((start_date - end_date).days / 30.44)
            else:
...
def analyze_worker(pipelineManager, selection, writer_queue, skills, stop_event):
    treated = 0
...
def start_analyze(pipelineManager, stop_event):
    ...
    writer_queue, stop_event_writer, writer_thread = start_writer()
    analyze_worker(pipelineManager, stop_event, writer_queue, skills, stop_event)  